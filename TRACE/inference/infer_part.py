...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=4096,
        help="Max prompt tokens for MeetingBank inference (long transcripts). Must match training.",
    )
    parser.add_argument(
        "--temperature",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.0,
        help="MoRAM inference: mask router weights where L2-normed rank projection < this (>=), all ranks. 0 disables.",
    )
    parser.add_argument(
        "--moram_topk",
        type=int,
        default=None,
        help="MoRAM router top-k override (default: use value from adapter_config.json / training).",
    )
...
def main():
    ...
    # Check if model is Qwen3 for specific generation parameters
    is_qwen3 = 'qwen3' in args.model_name_or_path.lower()
    
    # Qwen3 recommended: 16384 tokens for most queries
    # But use shorter lengths for classification tasks
    QWEN3_SHORT_TASKS = {"FOMC", "C-STANCE", "BoolQA", "QQP", "amazon", "yelp", "agnews", "dbpedia", "yahoo"}
    QWEN3_MEDIUM_TASKS = {"ScienceQA", "NumGLUE-cm", "NumGLUE-ds", "Py150"}
    # Long tasks: MeetingBank, 20Minuten (summarization/generation)
    
    def get_qwen3_max_tokens(task, default_max):
        """Get appropriate max tokens for Qwen3 based on task type."""
        if task in QWEN3_SHORT_TASKS:
            return min(256, default_max)  # Short classification answers
        elif task in QWEN3_MEDIUM_TASKS:
            return min(1024, default_max)  # Medium length for reasoning
        else:
            return min(2048, default_max)  # Longer for summarization tasks
    
    def prediction(model, infer_dataloader, current_task=None):
        predicted_sequences = []
        sources_sequences = []
        ground_truths = []
        model.eval()
        for step, batch in enumerate(infer_dataloader):
            ...
            progress_bar.set_postfix({
                'samples': len(predicted_sequences) + batch_size,
                'prompt_len': prompt_len
            }, refresh=True)
            with torch.inference_mode():
                # if is_qwen3:
                #     # Qwen3 recommended parameters:
                #     # Temperature=0.7, TopP=0.8, TopK=20, presence_penalty=0-2
                #     # Use task-appropriate max tokens (recommended: up to 16384)
                #     qwen3_max_tokens = get_qwen3_max_tokens(current_task, args.max_ans_len)
                #     generate_ids = model.generate(input_ids=batch['input_ids'],
                #                                   attention_mask=batch['attention_mask'],
                #                                   max_new_tokens=qwen3_max_tokens,
    ...
    if args.CL_method == "MoRAM":
        from utils.my_peft import PeftModel
        model = PeftModel.from_pretrained(model, inference_model_path)
        for name, param in model.named_parameters():
            if "lora_" in name or "loranew_" in name or "router" in name:
                param.requires_grad = False
        for module in model.modules():
            if hasattr(module, "set_router_temperature"):
                module.set_router_temperature(getattr(args, "moram_router_temp", 0.01))
            if hasattr(module, "set_moram_infer_lora_a_threshold"):
                module.set_moram_infer_lora_a_threshold(
                    getattr(args, "moram_infer_lora_a_thresh", 0.0),
                )
            if getattr(args, "moram_topk", None) is not None and hasattr(module, "moram_topk"):
                module.moram_topk = int(args.moram_topk)
    ...
    if "lora" not in str(args.CL_method).lower() and args.CL_method not in ['LFPT5', 'OGD', 'FIX', 'TaSL', 'SAPT', 'MoRAM']:
    ...
    # Always use bfloat16 for faster inference
    print("\033[34m" + "Converting model to bfloat16 for faster inference" + "\033[0m")
    model.to(dtype=torch.bfloat16, device=device)
    
    # Note: torch.compile() can cause overhead for some models, disabled by default
    # Uncomment to enable if beneficial for your model:
    # try:
    #     model = torch.compile(model, mode="reduce-overhead")
    #     print("\033[34m" + "Model compiled with torch.compile() for faster inference" + "\033[0m")
    # except Exception as e:
    #     print(f"\033[33m" + f"torch.compile() not available or failed: {e}" + "\033[0m")
    
    # Print generation parameters once
    print("\n" + "="*60)
    print("\033[32m" + "Generation Parameters:" + "\033[0m")
    print(f"  model: {args.model_name_or_path}")
    print(f"  is_qwen3: {is_qwen3}")
    if is_qwen3:
        print(f"  [Qwen3 Recommended Settings]")
        print(f"  temperature: 0.7")
        print(f"  top_p: 0.8")
        print(f"  top_k: 20")
        print(f"  do_sample: True")
        print(f"  repetition_penalty: 1.5")
        print(f"  max_new_tokens: task-aware (short:256, medium:1024, long:2048)")
        print(f"  chat_template: enabled")
    else:
        print(f"  temperature: {args.temperature}")
        print(f"  do_sample: False (greedy)")
        print(f"  repetition_penalty: 1.2")
        print(f"  no_repeat_ngram_size: 3")
        print(f"  max_new_tokens: {args.max_ans_len}")
    print(f"  max_prompt_len: {args.max_prompt_len}")
    print(f"  inference_batch: {args.inference_batch}")
    print(f"  CL_method: {args.CL_method}")
    print(f"  current_rank: {args.current_rank}/{args.total_rank}")
    print("="*60 + "\n")
    ...
    for inference_task_id in range(round + 1):  # evaluation for previous tasks in a single round
        ...
        if args.CL_method != 'FIX':
            inf_data_collator = DataCollator(
                ...
                inference=True,
                task=inference_task,
                model_name=args.model_name_or_path,  # For Qwen3-specific prompt formatting
                meetingbank_max_prompt_len=args.meetingbank_max_prompt_len,
            )
        else:
            # For FIX: ICL!
        ...
        part_length = math.ceil(len(infer_dataset) / total_rank) if total_rank > 0 else len(infer_dataset)
        
        # # DEBUG:
        # part_length = len(infer_dataset) // total_rank // 10
        
        start_index = current_rank * part_length
        end_index = min(start_index + part_length, len(infer_dataset))
        if start_index >= len(infer_dataset):
            subset_indices = []
        else:
        ...
        progress_bar = tqdm(
            ...
            leave=True,
            desc=f"[Rank {args.current_rank}] {inference_task}",
            unit="batch",
            ncols=100
        )
        
        # Inference !
        print(f"\033[33m[Rank {args.current_rank}] Starting inference for {inference_task}: {len(subset)} samples in {len(infer_dataloader)} batches\033[0m")
        sources_sequences, predicted_sequences, ground_truths = prediction(model, infer_dataloader, current_task=inference_task)
        progress_bar.close()
        print(f"\033[32m[Rank {args.current_rank}] Completed {inference_task}: {len(predicted_sequences)} predictions\033[0m\n")
