...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=4096,
        help="Max prompt tokens for MeetingBank (long meeting transcripts). Other tasks still use --max_prompt_len. Set lower if OOM; must fit model context.",
    )
    ...
    # Orthogonal regularization hyper-parameters (O-LoRA family)
    parser.add_argument('--orth_lambda1',
                        type=float,
                        default=0.5,
                        help='Weight applied to orthogonal loss between historical and current adapters')
    parser.add_argument('--orth_lambda2',
                        type=float,
                        default=0.0,
                        help='Weight applied to L2 regularization on current adapters')

    # MoRAM-specific hyper-parameters
    parser.add_argument('--moram_rank',
                        type=int,
                        default=8,
                        help='LoRA rank used by the MoRAM continual learning method')
    parser.add_argument('--moram_router_temp',
                        type=float,
                        default=0.01,
                        help='MoRAM softmax temperature τ on sparse top-k router logits (official default 0.01).')
    parser.add_argument(
        '--moram_topk',
        type=int,
        default=None,
        help='MoRAM router keeps max(topk,2) of L2-normalized rank activations; omit to use rank r.',
    )
    parser.add_argument(
        '--moram_infer_lora_a_thresh',
        type=float,
        default=0.0,
        help='MoRAM inference: zero router weights where L2-normed rank projection < this (>=), for both frozen and current-task ranks. 0 disables.',
    )
    parser.add_argument(
        '--moram_target_modules',
        type=str,
        default=None,
        help='MoRAM: comma-separated LoRA injection sites (submodule name suffixes), e.g. '
        '"q_proj,v_proj" or "q_proj,k_proj,v_proj,o_proj". '
        'If omitted, uses the built-in mapping for the base model type (llama: q/k/v/o_proj).',
    )
...
def main():
    ...
    if args.CL_method == "MoRAM":
        ...
        _mtm = getattr(args, "moram_target_modules", None)
        _moram_targets = None
        if _mtm is not None and str(_mtm).strip():
            _moram_targets = [x.strip() for x in str(_mtm).split(",") if x.strip()]
            if not _moram_targets:
                raise ValueError(
                    "--moram_target_modules must list at least one module name, or omit the flag for defaults."
                )
        _lora_kw = dict(
            task_type=TaskType.CAUSAL_LM,
            r=args.moram_rank,
            lora_alpha=max(1, 2 * args.moram_rank),
            lora_dropout=0.1,
            use_moram=True,
            moram_topk=args.moram_topk,
        )
        if _moram_targets is not None:
            _lora_kw["target_modules"] = _moram_targets
        peft_config = LoraConfig(**_lora_kw)
        model = get_peft_model(model, peft_config)
        for module in model.modules():
            if hasattr(module, "set_router_temperature"):
                module.set_router_temperature(args.moram_router_temp)
            if hasattr(module, "set_moram_infer_lora_a_threshold"):
                module.set_moram_infer_lora_a_threshold(args.moram_infer_lora_a_thresh)
        for name, param in model.named_parameters():
            if "loranew_" in name or "loranew_router" in name:
                param.requires_grad = True
                continue
