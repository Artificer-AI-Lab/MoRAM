...
def load_hf_tokenizer(model_name_or_path, fast_tokenizer=True):
    ...
    # Ensure we always have a pad token (needed for left padding + batching)
    if tokenizer.pad_token is None:
        ...
        # If the pad token does not exist yet, add it
        if pad_token not in tokenizer.get_vocab():
            tokenizer.add_special_tokens({'pad_token': pad_token})
        else:
            tokenizer.pad_token = pad_token

    # For llama (and similar causal models) we right-align inputs
    tokenizer.padding_side = 'left'
    tokenizer.truncation_side = "left"

    # Some llama tokenizers still miss bos_token
