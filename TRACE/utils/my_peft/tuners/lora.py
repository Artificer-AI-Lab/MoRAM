...
class LoraConfig(PeftConfig):
    ...
    use_moram: bool = field(default=False, metadata={"help": "Enable mixture-of-ranks adapters (MoRAM) instead of standard LoRA."})
    use_mora: bool = field(
        default=False,
        repr=False,
        metadata={"help": "Legacy adapter_config.json key; treated as use_moram when true."},
    )
    moram_topk: Optional[int] = field(
        default=None,
        metadata={
            "help": "MoRAM router top-k over rank experts (official: k=max(topk,r),2). None means use LoRA rank r."
        },
    )
...
class LoraModel(torch.nn.Module):
    ...
    **Attributes**:
    ...
    def _find_and_replace(self, adapter_name):
        ...
        use_moram = bool(
            getattr(lora_config, "use_moram", False) or getattr(lora_config, "use_mora", False)
        )
            ...
            if target_module_found:
                ...
                if isinstance(target, LoraLayer):
                    ...
                    if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
                        ...
                        if use_moram:
                            from .moram import MoRAMLinear

                            new_module = MoRAMLinear(
                                adapter_name,
                                in_features,
                                out_features,
                                bias=bias,
                                r_sum=lora_config.r_sum,
                                moram_topk=getattr(lora_config, "moram_topk", None),
                                **kwargs,
                            )
                        else:
                            new_module = Linear(
