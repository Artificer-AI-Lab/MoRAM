...
def infer_lora_r_sum_from_state_dict(state_dict, adapter_name="default") -> Optional[int]:
    """
    Read accumulated LoRA rank (lora_A out_features) from a saved adapter checkpoint.
    Saved files strip the adapter suffix (e.g. .default) from keys; support both layouts.
    """
    if not state_dict:
        return None
    for key, value in state_dict.items():
        if "loranew" in key or not isinstance(value, torch.Tensor):
            continue
        if ".lora_A.weight" in key or f"lora_A.{adapter_name}.weight" in key:
...
def get_peft_model_state_dict(model, state_dict=None, adapter_name="default"):
    ...
    if state_dict is None:
        ...
        if config.peft_type in PeftType.LORA:
            # Infer r_sum from actual lora_A weights (shape[0] is rank; 0 is valid for task 0).
            target_pattern = f"lora_A.{adapter_name}.weight"
            found_lora_a = False
            inferred_r_sum = 0
            for key, value in state_dict.items():
                if target_pattern in key and "loranew" not in key:
                    inferred_r_sum = int(value.shape[0])
                    found_lora_a = True
                    break
            if found_lora_a:
                config.r_sum = inferred_r_sum
            elif config.save_loranew is False:
