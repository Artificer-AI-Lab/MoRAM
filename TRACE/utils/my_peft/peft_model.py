...
from .utils.save_and_load import infer_lora_r_sum_from_state_dict
    ...
    **Attributes**:
    ...
    def load_adapter(self, model_id, adapter_name, is_trainable=False, **kwargs):
        ...
        if adapter_name not in self.peft_config:
            ...
            # adapter_config r_sum can disagree with tensors (e.g. bug when lora_A is empty); align from checkpoint
            if hasattr(peft_config, "r_sum"):
                inferred = infer_lora_r_sum_from_state_dict(adapters_weights, adapter_name)
                if inferred is not None:
                    peft_config.r_sum = inferred
            self.add_adapter(adapter_name, peft_config)
